...
$ret = `psql -qAtX -d regress_dbms_metadata -f test/sql/get_dependent_ddl/get_dependent_ddl_index_constraint.sql > results/get_dependent_ddl/get_dependent_ddl_index_constraint.out 2>&1`;
ok( $? == 0, "test for retrieving dependent index constraint ddl");

$ret = `psql -qAtX -d regress_dbms_metadata -f test/sql/get_dependent_ddl/get_dependent_ddl_sequence.sql > results/get_dependent_ddl/get_dependent_ddl_sequence.out 2>&1`;
ok( $? == 0, "test for retrieving dependent sequence ddl");

$ret = `psql -qAtX -d regress_dbms_metadata -f test/sql/get_dependent_ddl/get_dependent_ddl_trigger.sql > results/get_dependent_ddl/get_dependent_ddl_trigger.out 2>&1`;
